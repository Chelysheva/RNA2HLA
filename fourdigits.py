...
def determine4digits_main(file, bgVec, runName, HLAclass):
    dbmhc_file = ""
    dbmhc = []
    if HLAclass == 1:
...
def determine4digits(dbmhc, file, bgVec, runName):
    ...
    while repeat == 1:
        ...
        #there are more possible solutions
            ...
            if len(set(result)) == 1:
                #for item in result_dict:
                for item in sorted(result_dict2):
            ...
            #more than one 2 solution possible (at 4-digit level).
                ...
                ambiguity_handle = open(runName+".ambiguity.txt", "a")
                ...
                for ambi4DigitAllele in sorted(ambiguity_dict):
