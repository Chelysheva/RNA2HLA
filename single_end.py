...
def main_single(runName, readFile1, fastaClassI, fastaClassII, bowtiebuildClassI, bowtiebuildClassII, trim3, threads, genes, class1_list, class2_list, length_dict1, length_dict2):
    ...
    merge(runName)
#---------------Class I-------------------------------
def mainHLA(runName, readFile1, bowtiebuild, hla1fasta, mapopt, trim3, genes, gzipped, locus_list, hla_class, length_dict):
    twodigits1 = {}
    fourdigits1 = {}
    ...
    mapping(sam1, runName, readFile1, bowtiebuild, 1, mapopt, trim3, gzipped)
    medians = {}
    ...
    mapping(sam2, runName, newReadFile1, bowtiebuild, 2, mapopt, trim3, gzipped)
    ...
    try:
        if "no" in twodigits2.values():
            ...
            mapping(sam3, runName, newReadFile1, bowtiebuild, 2, mapopt, trim3, gzipped)
...
def mapping(sam, runName, readFile1, bowtiebuild, iteration, mapopt, trim3, gzipped):
    if iteration == 1:
        if gzipped == 0:
...
#Remove reads that mapped to the three top-scoring alleles and write the remaining reads into a new read file
