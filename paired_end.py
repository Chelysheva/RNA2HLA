# -*- coding: utf-8 -*-
"""
Ported to Python 3.7+ on Wed Jul 29 2026
...
#Title:
#Paired_end - dependent script from RNA2HLA tool.
#
#Synopsis:
#Script for HLA typing from paired-end RNA-seq sample.
#
...
#
#References:
#Developed based on from seq2HLA (release: 2.3) by Sebastian Boegel, 2012 - 2017 (c), BDOI: 10.1186/gm403.
#
#Usage:
...
def main_paired(runName, readFile1, readFile2, fastaClassI, fastaClassII, bowtiebuildClassI, bowtiebuildClassII, trim3, threads, genes, class1_list, class2_list, length_dict1, length_dict2):
    #define a type of input RNA-seq sample
    #print "Input is a paired-end RNA-seq sample..."
    ...
    try:
        ...
        process_wc = subprocess.Popen(['bash', '-c', cmd], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    mainHLA(runName+"-ClassI", readFile1, readFile2, bowtiebuildClassI, fastaClassI, mapopt, trim3, genes, gzipped, class1_list, 1, length_dict1)
    #call HLA typing for Class II
    print(str(numpy.datetime64('now')))
    mainHLA(runName+"-ClassII", readFile1, readFile2, bowtiebuildClassII, fastaClassII, mapopt, trim3, genes, gzipped, class2_list, 2, length_dict2)
    merge(runName)
#---------------Class I-------------------------------
def mainHLA(runName, readFile1, readFile2, bowtiebuild, hla1fasta, mapopt, trim3, genes, gzipped, locus_list, hla_class, length_dict):
    twodigits1 = {}
    fourdigits1 = {}
    ...
    mapping(sam1, runName, readFile1, readFile2, bowtiebuild, 1, mapopt, trim3, gzipped)
    medians = {}
    ...
    mapping(sam2, runName, newReadFile1, newReadFile2, bowtiebuild, 2, mapopt, trim3, gzipped)
    ...
    try:
        if "no" in twodigits2.values():
            ...
            mapping(sam3, runName, newReadFile1, newReadFile2, bowtiebuild, 2, mapopt, trim3, gzipped)
            ...
            if allele2 == "no":
                ...
                if allele3 == "no" or fourDigit_solutions1[locus] == 1 or not allele3.split(":")[0] == fourdigits1[locus].split(":"):
                    if fourDigit_solutions1[locus] == "":
                        ...
                        finalAlleles[locus] += fourdigits1[locus]+"\t"+str(getP(locus, finaloutput, genes))
                else:
...
def mapping(sam, runName, readFile1, readFile2, bowtiebuild, iteration, mapopt, trim3, gzipped):
    if iteration == 1:
        if gzipped == 0:
    ...
    #execute bowtie
    process_mapping = subprocess.Popen(['bash', '-c', mapping_cmd], stdout=subprocess.PIPE, text=True)
    out, err = process_mapping.communicate()

    if iteration == 1:
        #print alignment stats
        printcommand = "cat "+runName+".bowtielog"
        printcommand_proc = subprocess.Popen(['bash', '-c', printcommand], stdout=subprocess.PIPE, text=True)
...
def predictHLA(sam, medians, output, medianflag, locus_list, hla_class, readcount, readspergroup):
    ...
    for locus in locus_list:
        if len(readspergroup_dict_list[locus]) > 0:
            maxkey[locus] = max(sorted(readspergroup_dict[locus]), key=lambda a: readspergroup_dict[locus].get(a))
    ...
    for locus in locus_list:
        if not allele_dict[locus] == "no":
            ...
            if entries[entryIndex] != "NA":
                entries[entryIndex] = '%.12g' % (1-float(entries[entryIndex]))
        entryIndex += 2
...
def pred2File(entries, readspergroup_dict, output, allele_dict, HLAclass, locus_list):
    ...
    for locus in locus_list:
        ...
        out.write(str(numpy.median(list(readspergroup_dict[locus].values())))+"\t")
...
def expression(locus_list, length_dict, map, runName, readFile1, hla_class):
    ...
    print(logfile)
    #locate the "# reads processed" line robustly: newer bowtie 1.x versions (>=1.2)
    #print a deprecation warning as the first stderr line, so it is not always line 1
    totalreads = None
    with open(logfile) as loghandle:
        for logline in loghandle:
            if logline.startswith("# reads processed"):
                totalreads = float(logline.split(':')[1])
                break
    if totalreads is None:
        raise IOError("Could not find '# reads processed' line in "+logfile)
    alleles_in = runName+".digitalhaplotype1"
    alleles = []
    line = 2
    for locus in locus_list:
...
#(genes is threaded through explicitly here rather than relying on it being a module-level
#global set later in the __main__ block, which is what the original Python 2 script did)
def getP(locus, finaloutput, genes):
    if genes == 5:
...
if __name__ == '__main__':
    length_dict1 = {}
    length_dict2 = {}
    parser = OptionParser(usage="usage: %prog -1 readFile1 -2 readFile2 -f readFolder -r runName [-p <int>] [-3 <int>] [-g <int>]")
    parser.add_option("-1",
        action="store",
        dest="readFile1",
    ...
    readFile2 = (options.readFolder+"/"+options.readFile2)
    ...
    main_paired(runName, readFile1, readFile2, fastaClassI, fastaClassII, bowtiebuildClassI, bowtiebuildClassII, trim3, threads, genes, class1_list, class2_list, length_dict1, length_dict2)
